...
| 221 | `src/interface/plots.jl` | 32 | Plotting glue for the native engine: generic `plot_df` (dead) and the AA `_draw` overload for `ReactionNetworkProblem` plotting species trajectories from `prob.sol`. | reads `:specName` | defines `AlgebraicAgents._draw` | none (post-hoc viz) |
...
| 221 | `src/interface/solve.jl` | 202 | Result-side plotting: `@plot` macro + ensemble/ribbon/log-based helpers. Despite name + `export @agentize`, defines NO `@agentize`/`@solve`. STALE: reads `sol.prob.p[:__state__]`, references SciML `EnsembleSummary`/`EnsembleSolution`. | reads via stale `__state__` handle and `state` arg | only the orphan `export @agentize` | none (post-hoc viz; mostly broken vs native engine) |
