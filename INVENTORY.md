...
ReactiveDynamics.jl (RD) is NOT a chemical reaction network despite its DSL surface and Catalyst-derived parser. It is a timed, stochastic, resource-constrained Petri net / discrete-event system: transitions are stateful recipes that spawn instances at a Poisson rate, occupy resources (species) over a cycle time, and complete with a terminal Binomial probability-of-success that emits RHS products; resources carry a modality (`:nonblock`/`:conserved`/`:rate`) governing allocation, and a cost/reward/valuation ledger accumulates into a per-step `log`. On the `ref-agents` branch the package has fully pivoted from the old SciML embedding: the simulation output is now the NATIVE engine type `ReactionNetworkProblem` (an AlgebraicAgents `@aagent`, not a `DiscreteProblem`); AlgebraicAgents (AA) is reexported from `state.jl` and stepping is driven by AA's `simulate`/`_step!`. The static authoring/IR store is a dependency-free typed struct-of-columns — ACSets and Catlab were dropped entirely (ADR 0003), and the remaining ACSets-lineage vocabulary was retired by ADR 0015: the store type is now **`ReactionNetwork`** (not `ReactionNetworkSchema`), the authoring macro **`@reaction_network`** (not `@ReactionNetworkSchema`), the conventional variable **`net`** (not `acs`), the store verbs renamed to `nrows`/`row_ids`/`column`/`cell`/`find_rows`/`add_row!`/… and UNEXPORTED. A structured-token agent layer (`interface/agents.jl`) was added and `optim.jl` was removed. **NOTE (staleness):** the Orientation and Module Map sections below were authored against an earlier snapshot and still use the OLD ACSets-lineage names (`ReactionNetworkSchema`, `@ReactionNetworkSchema`, `acs`, `subpart`/`incident`/`nparts`, `BasicSchema`, `@reexport using GeneratedExpressions`) as if current — read those occurrences as HISTORY and map them through the ADR-0015 rename note in the "Schema / static store" section. This document is the canonical map of current source and SUPERSEDES the architecture map in `REVIEW.md`, which was written against an earlier (SciML/`DiscreteProblem`) snapshot; REVIEW.md's line numbers must not be trusted.
...
| 225 | `src/interface/solve.jl` | ~55 | **Phase-0.6 + WS-4 + WS-B2:** the dead SciML `@plot`/`plot_summary`/`plot_ensemble_sol`/`first_sol` were DELETED (ADR 0014 §A1); WS-4 then deleted the dangling `export @agentize`. **WS-B2 re-added `@agentize` as thin constructor sugar** — it expands to exactly one `ReactionNetworkProblem(net[, u0, p]; …)` call (no second construction path), with auto-naming from a bare-binding net (`@agentize net` ⇒ `name = "net"`). The `export @agentize` is co-located here (solve.jl:1). | none | expands to the constructor call | none |
...
| `@agentize` | macro | `solve.jl:13` | **implemented — thin constructor sugar (WS-B2)** — expands to exactly one `ReactionNetworkProblem(net[, u0, p]; …)` call (no second construction path); auto-names from a bare-binding net. Was REMOVED in WS-4 (dangling export); ADR 0001's "thin sugar OR delete" alternative, re-added per ADR 0012 | yes |
...
- **`@agentize`** (`solve.jl:1`) — the WS-4 dangling `export` was DELETED, then WS-B2 RE-ADDED the macro as thin constructor sugar (expands to `ReactionNetworkProblem(net[, u0, p]; …)`, auto-names from a bare binding). It is once again exported AND now defined; the exports-resolve guard pins it.
...
> **Renamed 2026-07-15 for ADR-0015 (post-ACSets naming).** The remaining ACSets-lineage vocabulary was retired: the store type `ReactionNetworkSchema` → **`ReactionNetwork`** and the authoring macro `@ReactionNetworkSchema` → **`@reaction_network`**; the conventional variable `acs` → **`net`** and the `ReactionNetworkProblem` field `.acs` → **`.network`**; the store fields `.parts`/`.subparts` → **`.counts`**/**`.columns`**. The shim verbs were renamed to store vocabulary AND UNEXPORTED (internal `RD.`-qualified): `nparts`→`nrows`, `parts`→`row_ids`, `dom_parts`→`col_row_ids`, `subpart`→`column`/`cell`, `set_subpart!`→`set_cell!`, `add_part!`→`add_row!`, `add_parts!`→`add_rows!`, `rem_parts!`→`rem_rows!`, `incident`→`find_rows`; `union_acs!`→`merge_networks!`, `build_acs_from_dict`→`build_network_from_dict`. Old names survive one release as deprecation shims. The **GeneratedExpressions** dependency and its brace-comprehension feature were dropped. The paragraphs below still use the OLD names — read them as history and map through this note.
