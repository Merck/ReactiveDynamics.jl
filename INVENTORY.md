...
| 225 | `src/interface/solve.jl` | 13 | **Phase-0.6 + WS-4:** now a comment-only file — no exports, no code. The dead SciML `@plot`/`plot_summary`/`plot_ensemble_sol`/`first_sol` were DELETED (ADR 0014 §A1); WS-4 then deleted the dangling `export @agentize` (the macro was never defined; agentization is implicit in the `ReactionNetworkProblem` constructor). | none | none | none |
...
| ~~`@agentize`~~ | macro | — | **REMOVED (WS-4)** — was a dangling `export` at `solve.jl:1`; the macro was never defined and was the only `agentize` token repo-wide. Agentization is implicit in the `ReactionNetworkProblem` constructor; AA `@agentize` sugar is explicitly-deferred future work (ADR 0012) | n/a (export deleted) |
...
- **`@agentize`** (`solve.jl:1`) — dangling `export` DELETED; macro was never defined (agentization is implicit in the constructor).
