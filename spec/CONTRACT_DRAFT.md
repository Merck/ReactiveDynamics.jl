...
## Glossary — the Petri-net term dictionary (see [ADR 0017](adr/0017-petri-net-vocabulary.md), amended by [ADR 0018](adr/0018-firing-vs-transition.md))
...
**A token is a discrete unit of resource sitting in a place** — the Petri-net sense, unrelated to language-model tokens. *Token* is canonical and is NOT renamed. Neither is *transition* — though [ADR 0018](adr/0018-firing-vs-transition.md) moved which object carries it: a **transition** is the static recipe (the `:T` rows, `state.transitions`) and a **firing** is one in-flight execution of it (the engine type `Firing`, the heap `state.ongoing_firings`), where the engine previously had those two exactly reversed.
...
| one in-flight execution of a transition | a **firing** — engine type `Firing`, heaped in `state.ongoing_firings` (§6.1) | classical / GSPN ([ADR 0018](adr/0018-firing-vs-transition.md)) |
...
Retired **firing/transition** names ([ADR 0018](adr/0018-firing-vs-transition.md), same release): the in-flight-instance type is `Firing`, retiring `Transition` (the one name here with a one-release forwarding alias); its heap is `state.ongoing_firings`, retiring `ongoing_transitions`; the compiled static transition table is `state.transitions`, retiring `transition_recipes`; that table evaluated for the current tick — what `state[i, :trans…]` reads — is `state.sampled_transitions`, which is where the old `state.transitions` field moved to; and a structured token's back-pointer is `bound_firing`, retiring `bound_transition`. The renamed *fields* carry no shim (a struct field cannot hold a forwarding binding), so model code that reads them updates in this release. Nothing else moves: the `:T` object, the `trans*` column symbols, the serialized keys (the document already said `"transitions"`) and the AA agent paths are unchanged.
...
The model has exactly one clock: a scalar simulation time `t` (runtime field `state.t::Float64`, `state.jl:48`). There is no per-transition or per-agent local clock; ongoing firings carry an *age* (`Firing.t`, the spawn time) and a *progress accumulator* (`Firing.state`), but both are read against the single global `t`.
...
An ongoing firing carries a progress accumulator `Firing.state` (initialized to `0.0` at spawn, `solvers.jl:187`). Each tick it advances by the fraction of its resource demand that was satisfied this tick, times `dt`:

```julia
transition.state += qs[i] * state.dt        # solvers.jl:261
```

where `qs[i] ∈ [0,1]` is the saturation (fraction satisfied) of that instance (`get_frac_satisfied`, `solvers.jl:95-103`). A firing is eligible to finish once `Firing.state >= transCycleTime` (`finish!`, `solvers.jl:409, :412`; survivors retained by `filter!(s -> s.state < s[:transCycleTime], …)`, `:501`).
...
- **Firing**: one in-flight execution of a transition (engine type `Firing`, `state.jl`; ADR 0018 named it — the static object is the transition, one execution of it is a firing), holding its parent transition-row index `i`, a frozen sampled-attribute snapshot `trans`, its birth time `t`, its multiplicity `q` (how many concurrent firings this instance object represents), and an accumulated progress `state`.
...
> **D7 (Re-init restores the stream).** `_reinit!` (`src/solvers.jl:619-628`) currently resets `u`, `t`, `ongoing_firings`, `log`, `observables`, and `sol`, but does NOT reset the RNG — so a second run after `reinit!` diverges. The contract requires `_reinit!` to restore the RNG to the exact state implied by the original `(M, seed)`, so that `init → step* → reinit! → step*` reproduces the first trajectory. The seed (or the initial RNG state) MUST be stored on the state for this purpose.
...
- **Soft-deactivate, not delete.** Logical removal of a transition is `transActivated[i]=false` (honored as a skip-gate at `state.jl:179`); the row, its index, and all higher indices stay put, and in-flight instances of a deactivated transition still run to completion (`finish!` iterates `ongoing_firings` independently of `transActivated`, `solvers.jl:406`).
...
> **Spec note — the live GROWTH half of ADR 0004 is SPECIFIED, NOT IMPLEMENTED ([issue #27](https://github.com/Merck/ReactiveDynamics.jl/issues/27)).** This paragraph, §7.5 J8, §8.6 S6 and the §10.1 lifecycle table name a surface that is only half-built, and the asymmetry is deliberate to record: the SOFT-DEACTIVATE half is real — `activate!`/`deactivate!` are defined and exported (`src/actions.jl:167,178`, export at `:16`), backed by the latching `transActivated` gate and the `Activate`/`Deactivate` actions, and the in-flight-survival rule (INV-3) holds as specified. The APPEND-ONLY GROWTH half is not: `add_place!`, `add_transition!`, `add_param!`, `refresh_wrap_fun!`, `apply_patch` and the `spec_delta` patch document have ZERO definitions in `src/` (verified by grep on this branch). `@add_place` (`src/interface/update.jl:246`) is NOT this API — it is an authoring-time macro that pushes rows into a `ReactionNetwork` before construction, and §7.5 J8 forbids authoring-time operations once the model is live. `add_structured_token!` (`src/interface/agents.jl:62`) IS real, but it grows the AA `inners` dict, never the positional `state.u` columns, so it does not exercise the frozen-varmap invariants. The normative content of these passages is UNCHANGED — this is what the engine must do; the marker records that a caller cannot call it yet, so the only way to change a running model today is to rebuild it from scratch, losing the RNG stream and the in-flight instances that append-only mutation exists to preserve. Implementation is DEFERRED, gated on a demo that actually exercises mid-run injection, so ADR 0004's INV-1…INV-4 get tested against a use rather than in the abstract (see [adr/README.md](adr/README.md), "Genuinely-deferred items").
...
The LHS/RHS arc specification is NOT a field here — it lives in the `ArcSpec` table (§6.5), which is the whole point of the Phase-2 promotion. The runtime in-flight firing (engine type `Firing`, `state.jl`, carrying `i`, frozen `trans` snapshot, birth `t`, multiplicity `q`, progress `state`) is a runtime object derived from this transition row, not a model object.
...
> **Spec note — the live GROWTH half of ADR 0004 is SPECIFIED, NOT IMPLEMENTED ([issue #27](https://github.com/Merck/ReactiveDynamics.jl/issues/27)).** J8's prohibition is normative and in force, but its escape route is only half-built: of the "*only* sanctioned way to change a live model" named here, `deactivate!` exists (`src/actions.jl:178`) while `add_place!`/`add_transition!` have zero definitions in `src/`. So today J8 leaves NO sanctioned way to GROW a live model — `@add_place` (`src/interface/update.jl:246`) is the authoring-time macro J8 itself forbids once the model is live. Deferred, gated as recorded in the §6.2 spec note.
...
> **Spec note — the live GROWTH half of ADR 0004 is SPECIFIED, NOT IMPLEMENTED ([issue #27](https://github.com/Merck/ReactiveDynamics.jl/issues/27)).** §8.6 is normative intent, not present behavior: `apply_patch`, the `spec_delta` patch document, `refresh_wrap_fun!`, `add_place!`, `add_transition!` and `add_param!` all have zero definitions in `src/`, so the acquisition lever described here has no on-disk form the loader will accept — the JSON surface that DOES round-trip is the full model document (§8.1–§8.5), not a patch. Two consequences worth stating plainly: (a) the LOAD-BEARING append-only argument of the paragraph above is a design constraint on the future implementation, not a check the engine performs; (b) the soft-deactivation escape (`transActivated[i] = false`, gated in `sample_transitions!` at `src/state.jl:307`, with in-flight instances running to completion — `finish!` at `src/solvers.jl:672` iterates `ongoing_firings` with no `transActivated` check) IS implemented and reachable through `deactivate!`/the `Deactivate` action — so a patch's *logical removal* semantics exist while its *growth* semantics do not. Deferred, gated as recorded in the §6.2 spec note.
...
| **Live / stepping** | constructed `ReactionNetworkProblem` | APPEND-ONLY (ADR 0004) | `simulate`/`step!`; `add_place!`/`add_transition!`/`add_param!` (NOT IMPLEMENTED — see the spec note below); `activate!`/`deactivate!`; `add_structured_token!`/`disentangle!`; `apply_patch` (§8.6, NOT IMPLEMENTED); query API (§9.3); `dump_state`/`reinit!` | any `rem_parts!` reindex (§7.5 J8); reordering/deleting object rows |

> **Spec note — the live GROWTH half of ADR 0004 is SPECIFIED, NOT IMPLEMENTED ([issue #27](https://github.com/Merck/ReactiveDynamics.jl/issues/27)).** In the Live row's "Legal operations", the callable-today set is `simulate`/`step!`, `activate!`/`deactivate!` (`src/actions.jl:167,178`), `add_structured_token!` (`src/interface/agents.jl:62`), `disentangle!` (AlgebraicAgents', called at `src/solvers.jl:1124` and `src/interface/checkpoint.jl:104`), the query API, and `dump_state`/`reinit!`. `add_place!`, `add_transition!`, `add_param!` and `apply_patch` are specified-not-implemented: they have zero definitions in `src/`, so the row states which operations are LEGAL in this phase, not which are AVAILABLE. The APPEND-ONLY mutability column and the Forbidden column are unaffected — both remain in force for the operations that do exist. Deferred, gated as recorded in the §6.2 spec note.
