...
- **Theory goes in its own section, never inline in a tutorial.** The "why" (Petri-net / discrete-event semantics, rNPV rationale) lives in the explanation layer (Workstream D) — matching JuMP's *Background* and ModelingToolkit's *Internals* split. Tutorials link to it; they don't embed it.
