...
- **[adr/](adr/)** — one Architecture Decision Record per real decision (0001–0015 plus 0017 and 0018 implemented; 0016 is drafted on another branch), append-only, with a status table in [adr/README.md](adr/README.md). ADRs record context/decision/consequences/rejected-options; the CONTRACT records the resulting semantics.
...
The modeling + analysis + visualization surface is BUILT and green. The full contract (§1–§15) and ADRs 0001–0015 plus 0017 and 0018 are implemented and tested; the suite runs FULLY green under `test/semantic/` — **834 pass / 0 broken / 834 total**, no `@test_skip` placeholders and no `@test_broken` pins remaining (the last 3 — the CONTRACT §1.4 construction-time modality validators — landed in `53d1fac`). AlgebraicAgents is the published registry release 0.4 (the earlier `Merck/AlgebraicAgents.jl@main` `[sources]` pin was dropped, `eb2ee10`).
...
| ADR 0017 **amendment** — finish the vocabulary at the arc/binding layers (`stoich`→`multiplicity`, `placeModality`→`placeDefaultModality`, `UnfoldedArc`→`ResolvedArc`, `bound_structured_agents`→`bound_tokens`, `structured_to_agents`→`binding`) + the drift rule and the `reaction`-is-notation rule, written into the ADR, the CONTRACT Glossary and `docs/src/glossary.md` | 0017 §Amendments | `src/state.jl`, `src/serialize.jl` (`multiplicity` wire key + `_legacy_key` read alias), `src/ledger.jl`, `src/ReactiveDynamics.jl` (`placeDefaultModality`); the retired-vocabulary grep gate is now executable in `test/semantic/exports_resolve.jl` (+7 asserts → 828) | `5224105` + the docs commit |
| Firing-vs-transition rename — the `@aagent` instance type is a **`Firing`**, the static rows are `state.transitions` (was `transition_recipes`), the per-tick evaluated snapshot they displace is `state.sampled_transitions`, a token's back-pointer is `bound_firing`, and every `Firing`-valued parameter is named `firing` (including the compiled closure's, with `@firing` as the canonical in-model marker) | 0018 | `src/state.jl`, `src/solvers.jl`, `src/interface/agents.jl`, `src/{ledger,predicates,actions,compilers,visualize}.jl` + `test/`/`spec/`/`docs/`/`demo/`; the type keeps a one-release alias `@deprecate_binding Transition Firing false` (`src/ReactiveDynamics.jl:530-542`), the renamed fields get none (a glossary row instead of a `getproperty` branch in the step loop) | the ADR-0018 commit on `rename/petri-vocabulary` |
...
- **Firing-vs-transition rename (ADR 0018) — DONE.** Accepted as Option C and landed with ADR 0017, so authors absorb one migration rather than two. The inversion is gone: a `Firing` is one in-flight execution, `state.transitions` is the static table. Two things the ADR's first draft got wrong and the implementation corrected: `transitions` was NOT a free name (it held the per-tick evaluated snapshot, now `sampled_transitions` — collapsing the two is a load-time `duplicate field name` error, which is how it was caught), and the blast radius was ~250 text sites, not 92. The four open questions are resolved inline in the ADR's **Resolutions** section; nothing is gated. Out of scope by design and still true: the `:T` object and its `trans*` columns (`src/operators/joins.jl` selects that family by the literal substring `"trans"`), the serialized keys (already `"transitions"`), and AA agent paths (built from `:transName`).

## Remaining work (all deferred; each with its gate — nothing non-deferred is open)

1. **`dump_state` in-flight limitation (implemented WITH a documented constraint, not a gap).** ADR 0007 §C `dump_state`/`restore` shipped, but `dump_state` REFUSES to dump when any transition is mid-cycle — it errors unless `ongoing_firings` is empty (`src/interface/checkpoint.jl:37-41`), i.e. dump is a clean-tick-boundary operation only (Milestone-1 scope). Serializing in-flight `ongoing` instances eval-free is the open ADR 0007 §C question; not a bug, an accurately-scoped limitation.
2. **Entity-level refinement (ADR 0009 §F) — deferred.** A structured token hosting its own sub-network. `refine`/`abstract`/`@compose`/`@pipeline`/`@process` (transition-level refinement + composition) all shipped; only the entity-hosts-a-subnetwork axis is deferred to a future ADR. §A of ADR 0009 was designed so as not to preclude it.
3. **Threaded ensemble backend — deferred.** `ensemble(...; parallel = true)` is accepted but currently runs sequentially (`src/analysis.jl:294`); a real threaded backend is future work, orthogonal to mode (b) (members are independent, so it is a safe extension).
4. **AA `Opera`-level implicit/fixed-point (algebraic-loop) coupling — deferred.** Current AA coupling is explicit one-tick-lag Jacobi (`src/interface/aa_coupling.jl:123`); a within-tick fixed point is a separate AA-level `Opera` design (noted in ADR 0012).
5. **ADR 0003 Phase 3 (`to_acset` weakdep interop view) — REJECTED / will-not-do (2026-07-15).** No AlgebraicJulia/ACSets/Catlab interop view will be built; interop is off the BD/rNPV roadmap and impossible for the running stateful engine. This is a definitive rejection, not a parked/deferred option — any future interop would be a new ADR against the then-current engine (the dependency-free typed IR is the permanent core). Recorded in ADR 0003 Resolved. NOT counted as remaining work.
6. **Follow-up naming pass (`dt`/`tstep`) — DONE 2026-07-16.** CONTRACT_DRAFT flagged `tstep` as an internal alias / rename candidate; explicitly out of scope for ADR 0015, folded into this separate pass. Landed: the internal keyword-bag key and the accepted keyword now both use `dt` (matching the `state.dt` field, reconciled by name not position); the legacy `tstep` meta keyword is an accepted-but-deprecated alias mapped to `dt` with a depwarn for one release. Also fixed a latent silent-ignore bug (a user-supplied `tstep` was previously overwritten before use).
7. **Tutorial/documentation rework — IN PROGRESS on branch `docs-tutorials` (standalone PR into `rework`).** Chartered and tracked in **[DOCS_CHARTER.md](DOCS_CHARTER.md)**: tiered onboarding tutorials (introductory/advanced/expert) + technical deep-dives (serialization, composition), three applied decision case studies (marginal-scientist flagship, in-licensing BD/M&A, kill-a-program), a rebuilt capability-organized API reference (the current `docs/src/index.md` references removed macros), and — for the "why" — links to the normative contract/ADRs plus two companion papers (a methodology paper and an HBR-style adoption/value paper; the on-site explanation quadrant D1/D2 was dropped 2026-07-21) — all literate, on a Documenter+Literate site, with the seven `demo/` tours migrated in place as the single source of truth. First increment landed: the charter + a fully-rendered introductory-tutorial exemplar (`docs/literate/tutorials/introductory.jl`) + the Documenter+Literate build scaffold (`docs/make.jl`, `docs/Project.toml`, `docs/build_literate.jl`). Remaining facets tracked in the charter's §10.
