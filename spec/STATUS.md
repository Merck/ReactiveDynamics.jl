...
- **[adr/](adr/)** — one Architecture Decision Record per real decision (0001–0015 plus 0017 implemented, 0018 proposed; 0016 is drafted on another branch), append-only, with a status table in [adr/README.md](adr/README.md). ADRs record context/decision/consequences/rejected-options; the CONTRACT records the resulting semantics.
...
The modeling + analysis + visualization surface is BUILT and green. The full contract (§1–§15) and ADRs 0001–0015 plus 0017 are implemented and tested; the suite runs FULLY green under `test/semantic/` — **828 pass / 0 broken / 828 total**, no `@test_skip` placeholders and no `@test_broken` pins remaining (the last 3 — the CONTRACT §1.4 construction-time modality validators — landed in `53d1fac`). AlgebraicAgents is the published registry release 0.4 (the earlier `Merck/AlgebraicAgents.jl@main` `[sources]` pin was dropped, `eb2ee10`).
...
2. **Firing-vs-transition rename (ADR 0018) — PROPOSED, gated on a release question.** The `@aagent` instance type is named `Transition` but IS a firing; the static transitions sit in `transition_recipes`, an invented word coined to escape that collision — canonical usage exactly reversed. [ADR 0018](adr/0018-firing-vs-transition.md) recommends `Transition`→`Firing`, `ongoing_transitions`→`ongoing_firings`, `transition_recipes`→`transitions` (92 text sites). Deliberately NOT in scope: the `:T` object and its `trans*` columns (`src/operators/joins.jl:40` selects that column family by the literal substring `"trans"` — the same reflection trap ADR 0017 hit at `place`), the serialized keys (already `"transitions"`), and AA agent paths (built from `:transName`, not the Julia type). No shim set and no wire-format change are needed, so the only open decision is whether it lands in the same release window as ADR 0017's shim removal or the one after — maintainer's call.
