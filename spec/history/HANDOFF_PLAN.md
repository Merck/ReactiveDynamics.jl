...
> **⚠️ SUPERSEDED / HISTORICAL (archived 2026-07-15).** Every workstream in this plan (WS-1 store swap, WS-2 refinement/composition, WS-3 pinned bugs, WS-4 housekeeping, WS-5 introspection tour) has SHIPPED and is green. This file is retained only as a record of how that work was scoped and sequenced. For the CURRENT state and what genuinely remains, see [../STATUS.md](../STATUS.md). Do not treat any `file:line`, test tally, or "NOT landed / Proposed / gated" claim below as current — they were true on 2026-07-09 and have all since been resolved.

> **Purpose (as written 2026-07-09).** A self-contained, agentic-handoff plan for the work still open on the `ref-agents` revision as of 2026-07-09. It assumes NO prior conversation context. Read this top to bottom, then verify current state (§0) before touching code — every `file:line` and test tally below drifts and must be re-checked. Source of truth for decisions: `../adr/`; normative spec: `../CONTRACT_DRAFT.md`; the current-source map: `../../INVENTORY.md`. The Phase-0 sign-off and the already-landed Phase-0.6 increment were recorded in the since-retired `PHASE0_REVIEW.md`; that state now lives in [../STATUS.md](../STATUS.md) and the ADRs.
