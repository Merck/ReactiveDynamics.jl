...
|---|---|
| `:S` species | `placeName`→Symbol, `placeModality`→`Set{Symbol}`, `placeInitVal`/`placeInitUncertainty`/`placeCost`/`placeReward`/`placeValuation`→SampleableValues, `placeStructured`→Bool, `placeRole`→Symbol (ADR 0009 §A port role, default `:private`) |
...
Supporting root tables/routines: `prettynames` (user alias → canonical attr), `defargs` (per-object defaults, incl. `:placeStructured => false`, `:placeRole => :private`), `species_modalities` (`[:nonblock, :conserved, :rate]`), `assign_defaults!`, `add_obs!`, `merge_networks!`, the constructors. String→attribute coercion is `convert(Symbol, String)` on assignment (the eval-based Set/FoldedObservable convert hooks were removed with ADR 0005).
