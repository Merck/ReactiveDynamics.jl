...
This document maps the CURRENT source on branch `rework`. Everything specified in the contract (§1–§15) and ADRs 0001–0015 plus 0017 and 0018 is implemented and green — the semantic suite runs **835 pass / 0 broken** locally with a Graphviz backend installed, **833 pass / 1 environment-skipped / 834 total** on CI (Graphviz is a system binary the runners do not install; see `spec/STATUS.md` for the feature→commit table). Re-verify any `file:line` before acting — line numbers drift; grep to confirm.
...
| `src/interface/reaction_parser.jl` | 154 | Arc extraction (from Catalyst): folds a reaction side into `FoldedArc`s carrying place/multiplicity/modality; parses the `@select(Kind, clauses)` LHS into a `TokenPredicate` (`:72-101`, against `PRED_OP_WHITELIST` from `predicates.jl:23`); resolves `@choose`, escapes refs. | reads `net[:, :placeName]` | indirect (consumes state `getindex`) | sampling/parse-support |
...
| `src/interface/checkpoint.jl` | 131 | State dump/restore (ADR 0007 §C / CONTRACT §10.5): `StateDump`/`dump_state`/`restore` serialize a live run at a TICK BOUNDARY into an eval-free JSON-representable artifact (initial marking + dynamic state: clock, RNG, creation counters, `u`, token population, rule latches). `dump_state` refuses mid-cycle (errors unless `ongoing_firings` empty) — a scoped Milestone-1 constraint. | reads/reconstructs the store + run-state | dumps/restores the AA token population | boundary-only (not in the hot loop) |
...
| `src/interface/update.jl` | 584 | UPDATE half of the DSL: mutation/config macros (`@push`, `@add_place`, `@mode`, `@name_transition`, `@prob_init`/`@prob_uncertainty`/`@prob_params`/`@prob_meta`, `@cost`/`@reward`/`@valuation`, `@periodic`/`@jump`, `@aka`, `@register`) that reshape syntax into store mutations. | heavy reader/mutator | none | seeds step inputs (`@jump`/`@periodic`/`@prob_meta`) |
| `src/utils/utils.jl` | 139 | AST/kwarg grab-bag (`macroname`, `blockize`, `underscorize`, `args_kwargs`, …). NO exports. | none | none | none |
| `src/operators/equalize.jl` | 106 | Place identification/collapse (authoring-time): `equalize!` merges matched place rows + rewrites refs and re-derives the `ArcSpec` table; `@equalize` parses `A = B = C` blocks. | heavy mutator | none | none (model transform) |
| `src/operators/joins.jl` | 314 | Model composition (authoring-time): `merge_networks!` merges one network into another in place (S/P/M/E/obs by name, T appended); `@join` builds + folds models. | heavy reader/mutator | none | none (model transform) |
| `src/operators/refine.jl` | 432 | Hierarchical refinement & open-port composition (ADR 0009 / CONTRACT §11): `refine`/`refine!`/`abstract`/`abstract_transitions` (boundary-matched FK-splice, substitutable granularity), `set_port_role!`/`@port` (place port roles), `@compose`/`compose`, `@pipeline`/`@process` compact authoring, `refinement_diagnostics`. | reads/rewrites the store + `ArcSpec` FKs | none | none (authoring-time transform) |
| `src/solvers.jl` | 1240 | CORE discrete-event engine: the `ReactionNetworkProblem` constructor (`:923`) + the per-tick step loop `_step!` (`:1180`), the ADR-0002 weighted-progressive-fill allocator (`build_requirements!` `:100`, `progressive_fill!` `:162`, `spawn_integer!` `:268`), `evolve!` (`:305`) / `finish!` (`:672`) / `free_blocked_places!` (`:813`), structured-token RHS (`structured_rhs` `:563`), construction-time modality validation (`validate_modalities` `:860`), `get_tcontrol` (`:830`), and the AA contract `_reinit!`/`reinit!`/`_projected_to` (`:1090`/`:1160`/`:1229`). | heavy reader/writer | implements the AA stepping contract | THIS IS THE STEP LOOP |
| `src/predicates.jl` | 110 | Token filtration (ADR 0008 / CONTRACT §9.5): `TokenPredicate{kind, clauses}` (+ `Clause`, `matches`) narrows the candidate set a structured LHS binds; `select_tokens` (`:73`) filters then sorts by `token_sortkey` (`:107`, the `(place, creation_index)` total order, §4 D4). Phase is an ATTRIBUTE (`@select` selects, `@advance` advances) — no per-phase kinds. | reads token/place attrs | operates over structured-token agents | binding-time filter in the step loop |
| `src/actions.jl` | 325 | The endogenous decision channel (ADR 0010/0011 / CONTRACT §12): the closed `ActionStmt` family `{SetMarking, SetParams, SetField, SetTokens, AddToken, Activate, Deactivate, Invoke, Log, Seq}` + the `Rule{guard, action, fire_mode}` record; `fire_rules!` runs once/tick at step 10, `apply_action!`, `activate!`/`deactivate!`/`set_guard!`. `Invoke` is the eval-free general-code escape hatch (registry-by-name). | reads/writes place markings/params/tokens | none directly | fires at a fixed point in `_step!` |
...
| `src/serialize.jl` | 1135 | Single-JSON, eval-free model serialization (ADR 0005): `from_json_model` (`:225`) parses one JSON object → validates (`_validate_node!` `:666` / `_validate_action!` `:843`) → lowers each `ExprNode` dict via `to_expr` into the same `Expr` columns the DSL fills → constructs a `ReactionNetworkProblem`; `to_json_model` (`:328`) is the inverse; `build_network_from_dict` (`:131`), `node_to_dict`/`node_from_dict`/`model_to_dict`. NEVER `Meta.parse`s/`eval`s a model field (closes the import-time RCE). | writes the store from parsed IR | constructs the AA problem | none (authoring/serialization boundary) |
...
> Line counts are `wc -l` as of this refresh; `file:line` citations are the current 1-indexed positions. `optim.jl` was removed (include commented at `src/ReactiveDynamics.jl:473`); its old `@optimize`/`@fit`/`@fit_and_plot`/`build_parametrized_solver`/`optim!` symbols are gone.
...
| `exprnode.jl:128-129` | `ExprNode`, `Const`, `NodeRef`, `Call`, `Sample`, `TimeRef`, `Choose`, `Field`, `ExternalRef`; `OP_WHITELIST`, `DIST_WHITELIST`, `REF_KINDS`, `to_expr`, `from_expr` |
| `agents.jl:1-3,73,174` | `AbstractStructuredToken`, `BaseStructuredToken`, `@structured_token`, `register_token_kind!`, `add_structured_token!`, `PopulationEntry`, `log_token_fields` |
| `create.jl:3,6,7` | `@reaction_network`, `@ReactionNetworkSchema` (deprecated alias), `@append_transitions` |
| `update.jl:3-7,199` | `@push`, `@name_transition`, `@mode`, `@add_place`, `@periodic`, `@jump`, `@prob_init`, `@prob_uncertainty`, `@prob_params`, `@prob_meta`, `@aka`, `@register`, `@cost`, `@reward`, `@valuation` |
...
- **`struct ReactionNetwork`** (`:149`) — the static network container: `counts::Dict{Symbol,Int}` (rows per object), `columns::NamedTuple` (typed `AttrColumn`s in `ALLATTRS` order), `arcs::Vector{ArcSpec}` (the promoted incidence table). A typed inner constructor (`:157`) prevents the auto field-ctor from colliding with the legacy 3-arg semantic constructor. Content-based `Base.hash` (`:222`) EXCLUDES the derived `arcs` table so the model hash is stable across promotion. `Base.@deprecate_binding ReactionNetworkSchema ReactionNetwork` (`:172`) keeps old annotations compiling for one release.
- **Store verbs (UNEXPORTED, `RD.`-qualified):** `nrows` (`:236`), `row_ids` (`:237`), `col_row_ids` (`:238`), `getindex`/`setindex!` in 4 forms (`:241-247`), `column`/`cell`/`set_cell!` (`:250-252`), `find_rows` = `findall(isequal)` (`:256`, a `findall`, NOT an FK follow), `add_row!` (`:259`)/`add_rows!` (`:270`)/`rem_rows!` (`:282`). `[:,attr]`/`column` return COPIES. `rem_rows!` is SWAP-AND-POP (last row fills each freed slot; `equalize!` relies on the surviving-row order).
- **Port roles (ADR 0009 §A):** `const PORT_ROLES = (:private, :input, :output, :shared)` (`:200`); `port_role(net, i|name)` (`:208-213`) reads the `placeRole` attr (default `:private`).

Object/attribute model (element types, from `SCHEMA`):

| Object | Attributes (col → type) |
|---|---|
| `:S` places | `placeName`→Symbol, `placeDefaultModality`→`Set{Symbol}`, `placeInitVal`/`placeInitUncertainty`/`placeCost`/`placeReward`/`placeValuation`→SampleableValues, `placeStructured`→Bool, `placeRole`→Symbol (ADR 0009 §A port role, default `:private`) |
...
**Promoted arc relation (ADR 0003 Phase 2):** `struct ArcSpec` (`:135`) — `trans::Int` (FK→`:T`), `place::Int` (FK→`:S`, 0=dynamic), `multiplicity` (the arc weight), `side::Symbol`, `modality::Set{Symbol}`, `expr` (escape-hatch for a dynamic arc) — a `Vector{ArcSpec}` FIELD on the store (NOT a 7th SCHEMA object, so it stays out of the reflection loops). Derived by `populate_arcs!` (serialize.jl) from the `:trans` column via the eval-free static arc decomposition; `equalize!`/`refine` re-derive it post-transform for a structural FK-repoint. Accessors: `arcs`, `placename`, `find_index` (exported from the root).
...
3. `free_blocked_places!` (`:813`) — release `:nonblock` resources.
4. `update_u_structured!`.
5. `update_observables` — resample observables whose interval elapsed.
6. `sample_transitions!` (`state.jl:295`) — clear `state.transitions`, eval activated recipes (guard AND-ed with the `transActivated` latch), unfold LHS into `ResolvedArc`.
7. `evolve!` (`:305`) — SPAWN + ALLOCATE (progressive-fill) + advance ongoing (detail below).
8. `update_u_structured!`.
9. `finish!` (`:672`) — TERMINATE + PoS + emit RHS + return `:conserved`/`:nonblock` + prune (detail below).
...
Termination/progress: `_projected_to` (`solvers.jl:1229`) returns `true` once `state.t > state.tspan[2]`, else `state.t`.
...
### Spawning + completion
- `evolve!` (`:305`): per transition, compute the (Poisson/deterministic) desired count, apply `transCapacity`, allocate init requirements via the progressive-fill allocator, floor to whole firings, construct `Firing` heap entries (`t=state.t`, `q`, `state=0.0`), bind structured tokens in `select_tokens`/`token_sortkey` order, run `transPreAction`; ongoing instances then advance by `q*dt`.
- `finish!` (`:672`): terminate on lifetime-exceeded OR cycle-complete; draw `Binomial(q, PoS)` success; emit RHS products (incl. `structured_rhs` `:563` for `@structured` genesis); return `:conserved`/`:nonblock` resources; run `transPostAction`; prune terminated instances.

### Construction-time modality validation
`validate_modalities(net)` (`solvers.jl:860`) runs in the constructor BEFORE any closure compiles or tick runs, rejecting the three illegal §1.4 configs (`{:nonblock, :conserved}`; `:rate` with a concrete `cycletime == 0`; `:rate` on a structured place) with a clear `ArgumentError` (CONTRACT §1.4).

### Clock / dt notes
- Single authoritative clock `state.t`, advanced only at the end of `_step!`.
- The runtime field is `dt` (`state.jl`), and the accepted meta keyword is `dt`; the legacy `tstep` meta keyword is an accepted-but-deprecated alias mapped to `dt` (depwarn, one release). `get_tcontrol` (`:830`) resolves `tspan`/`dt` from `@prob_meta` or the constructor kwargs.
...
### `@aagent` structs (5) + abstract types
| Type | Location | Notes |
|---|---|---|
| `Firing` | `state.jl:49` | one in-flight execution of a transition; holds its binding |
| `Observable` | `state.jl:74` | per-observable sampler agent |
| `ReactionNetworkProblem` | `state.jl:86` | the top-level agent = the engine state |
| `EnsembleProblem` | `analysis.jl:262` | ensemble runner as a drawable/walkable AA node |
| `BaseStructuredToken` (`@aagent FreeAgent`) | `agents.jl:17` | base structured-token struct |
| `AbstractStructuredToken <: AbstractAlgebraicAgent` | `agents.jl:10` | root of the token hierarchy (abstract type) |

`@aagent` injects `(uuid, name, parent, inners, relpathrefs, opera)` and a name-first constructor; user token subtypes are generated programmatically via `AlgebraicAgents.aagent(...)` (`agents.jl:47`).

### AA interface methods implemented
| Method | Type | Location |
|---|---|---|
| `_step!` | `ReactionNetworkProblem` | `solvers.jl:1180` |
| `_reinit!` / `reinit!` | `ReactionNetworkProblem` | `solvers.jl:1090` / `:1160` (`reinit!` takes a `seed=` kwarg for ensemble mode (b) reseed) |
| `_projected_to` | `ReactionNetworkProblem` | `solvers.jl:1229` |
...
| `_step!` / `_projected_to` (no-op) | `AbstractStructuredToken` | `agents.jl:155` / `:154` |
...
- Structured tokens are first-class AA agents under a `"structured"` `FreeAgent` container; attached via `entangle!` in `add_structured_token!` (`agents.jl:63`); the solver reaches them via `inners(getagent(state, "structured"))`. The declarative `population[]` initial marking is instantiated at construction by `instantiate_population!` (`agents.jl:102`) in DECLARED order, assigning per-place creation indices through the seeded RNG (ADR 0007 §B). A token's `bound_firing` points to its in-flight `Firing`; the network↔token link is the AA `parent`/`inners` hierarchy.
