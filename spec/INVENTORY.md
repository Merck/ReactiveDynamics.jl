...
| `src/compilers.jl` | 205 | Expression-compilation layer: rewrites DSL attribute exprs (rates/multiplicity/actions/observables) into runnable `(state, transition)` closures; place→`u[i]`, params→`p[:name]`, query markers→state calls; bulk `compile_attrs`. | reads store attrs | none | setup (builds `transition_recipes`); `wrap_expr` also fires per-step via `state.wrap_fun` |
