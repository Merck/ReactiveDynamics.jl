...
- The dynamic escape-hatches are `@structured`/`@move` (recognized as macrocalls at `reaction_parser.jl:67` and passed through at `create.jl:300`) and `@choose` (a separate branch, recognized at `reaction_parser.jl:13` inside `recursively_choose` and at `create.jl:292`). These are the only legitimately dynamic reactant idioms and must become explicit typed nodes rather than arbitrary code. **PARTIALLY IMPLEMENTED (`@structured` genesis):** `@structured` now has a NAMED typed form `@structured(:Kind, field = node, …)` — an RHS genesis product carrying the registry kind + field-value ExprNodes, resolved by name through the registry at firing time (the RHS-product twin of `AddToken`, sharing its `(state, fields::Dict) -> token` contract). It round-trips through `to_json_model`/`from_json_model`/`validate` as a `reactants[]` row `{"side":"rhs","structured":{"kind","fields":[{name,value}]}}` (`structured_rhs` in `solvers.jl`; `_reactant_atom`/`_emit_macro_reactant!`/`validate` in `serialize.jl`; test `test/semantic/structured_genesis.jl`; demo `demo/agentic_pipeline` §4). The RAW form `@structured(Ctor(…))` — an inline host `Expr` carrying the constructor — was REMOVED, not retained: it was the only reactant construct that could not round-trip eval-free, so forbidding it makes eval-free serialization a TOTAL invariant (every model the engine accepts exports; there is no serializable/non-serializable split to police). A raw constructor on an `@structured` RHS is rejected at CONSTRUCTION (`recursively_find_reactants!`, `create.jl` — fail fast at model build, not deep in a simulation), with a message pointing at the named form. This supersedes an earlier design that kept the raw form as an escape hatch guarded by a loud `to_json_model` failure + an `unserializable_transitions` pre-flight; both were dropped once the raw form itself was removed. No implicit raw→named cast was ever offered: the registry key and field names are not recoverable from a raw constructor call, and the raw body need not even be a constructor call. `@move`/`@choose` remain Expr-level (the whole-line `ReactantSpec` escape hatch, `serialize.jl` `populate_reactant_specs!`).
