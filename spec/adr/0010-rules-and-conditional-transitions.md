...
- **Token-field-write action verb + general code (MVP finding I) — RESOLVED by [ADR 0011](0011-action-callbacks-and-general-code.md).** The gap (no Rule action could write a token field over a `@select`-ed set, and the closed verbs could not run arbitrary callback logic) is closed by ADR 0011, which extends the §12.3 family with `SetTokens{predicate, assigns}` (the eval-free population-level field write, legal in a Rule because it carries its own `TokenPredicate`) and `Invoke{fn, args}` (the general-code escape hatch, lowering to `registry[fn](state, transition, …)` via the ADR-0006 §C registry-by-name boundary, so the file stays inert). The canonical action family is now `{SetSpecies, SetParams, SetField, SetTokens, AddToken, Activate, Deactivate, Invoke, Log, Seq}`.
