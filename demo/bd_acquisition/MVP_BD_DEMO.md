...
| **Pipeline phase** | a `phase` ATTRIBUTE value `∈ {:Discovery, :Phase1, :Phase2, :Phase3, :Filed, :Market}` on the single `:Project` KIND (§9.5/ADR 0008 §D, the maintainer-canonical encoding) | not a separate place kind; portfolio counts per phase come from `@select`/`ntokens` reductions (§9.3) over the `phase` field |
...
| **Scientists / capacity** | plain `@conserved` place (held during a phase, returned on completion) | the scarce resource the ADR-0002 allocator rations by `priority` |
| **Budget / cash** | plain `@rate` (burn) + `@conserved` reserve place; replenished by a `scheduled` financing transition or a `every_tick` rule | drives the capital ledger (`:valuation_cost`, solvers.jl:308-311) |
| **Initial portfolio** | declarative `population[]` array in the model JSON (CONTRACT §10.3): `{place, kind, instances:[…]}` of `ProjectToken`s across phases | makes the starting pipeline part of the reproducible input (§8.2 S2), not imperative host code — closes the gap noted at finding **H** |
| **The acquisition** | **endogenous**: an ADR-0010 **rule** whose action is `Seq[AddToken(ProjectToken, …), SetParams(synergy…), SetMarking(scientists,+…)]`, `fire_mode: once`. The lever lives *in the model JSON* and may be scheduled (`@t() > T_acq`) **or** state-contingent (`portfolio_proxy < X`) | NOT the legacy host patch and NOT the old no-op event channel; `AddToken` lowers to `add_structured_token!` (index-safe, ADR 0006 §9.1) |
...
2. **Resource synergy** — the target brings headcount/capital: the rule's `SetMarking(scientists, +Δ)` / `SetParams(budget_rate, ·)` bumps the pools at acquisition.
...
| Declarative initial portfolio | `population[]` array (`{place,kind,instances}`) — §10.3, ADR 0007; makes the start pipeline part of the reproducible input (§8.2 S2) |
...
| **Acquisition lever (endogenous)** | **ADR 0010 rule**: guard ExprNode + `Seq[AddToken, SetParams, SetMarking]`, `fire_mode: once` — CONTRACT §12 |
...
**B. No endogenous, state-contingent decision channel — only exogenous host patches. — RESOLVED (ADR 0010 + §12).** Events were a no-op (Inv-7, solvers.jl:323) and §8.6 made the lever a host-applied patch — scheduled only, never "acquire *if* portfolio rNPV drops below X." ADR 0010 repairs the event channel into a **Rule** (guard ExprNode + closed action set), adds a stateless **`guard::ExprNode`** on transitions (AND-ed with the latching `transActivated`, state.jl:179), extends the action set to the full endogenous set `{SetMarking, SetParams, AddToken, Activate, Deactivate, Log, Seq}`, and adds `fire_mode {every_tick, once}`. The acquisition lever is now in-model, serializable, reproducible, and LLM-authorable.
...
3. **Action expressivity:** actions/callbacks may write a `@select`-ed token population (`SetTokens`) and run general host code (`Invoke`, via the eval-free ADR-0006 registry) — ADR 0011 + §12.3 (findings I + the general-code ask). Full family `{SetMarking, SetParams, SetField, SetTokens, AddToken, Activate, Deactivate, Invoke, Log, Seq}`.
