# Core Engine Tour (classical place)
...
These are deliberate, narrated in-line so a reader does not trip over them: `@rate` requires `cycletime > 0`, and `cycletime = 0` is now a construction-time `ArgumentError` rather than a silent no-op — CONTRACT §1.4, `validate_modalities` in `src/solvers.jl` (§3d shows the error and both repairs); `@nonblock` also wants `cycletime > 0`, but that one is NOT validated and its per-step credit-back is silently a no-op at `cycletime = 0`; a FRACTIONAL `@deterministic` count on a source is NOT dt-invariant, so use integer deterministic counts for sources (§5a note); and `@join` currently merges place / transitions / params but NOT observables (`:obs`) or events (`:E`) of the joined submodels (§7).
