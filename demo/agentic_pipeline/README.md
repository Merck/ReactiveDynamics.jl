...
| 5 | Model-as-data (eval-free JSON) | The same pipeline as a JSON document: `validate` (clean pass + a deliberately broken model yielding a diagnostic), `from_json_model` matching the DSL build bit-for-bit under the same seed, the security point (a malicious string param loads as inert data, never executed), and loading the document from a file with `@import_model`. The INVERSE now works too: `to_json_model`/`@export_model` emits a *live* model back to a full eval-free document, and reloading it reproduces the same trajectory under the same seed (idempotent round-trip) — a model is DATA in both directions (author-as-JSON → load, and build/load → export → reload). The run's OUTPUT (the solution trajectory via `@export_solution_as_table`) is a separate artifact from the model document. | [0005](../../docs/adr/0005-serialization-json-ir.md) |
