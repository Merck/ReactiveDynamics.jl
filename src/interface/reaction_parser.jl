# Reaction-line parser: lower a `:trans` reaction line into its arc terms. Parts of this file were
...
recursively). Called by [`extract_arcs`](@ref) before the line is unfolded into arc terms.
"""
...
"""
Parse a reaction line `r_line` into its `Vector{FoldedArc}` against the live `state`: first resolve
any `@choose` ([`recursively_choose`](@ref)), escape indexed place names ([`escape_ref`](@ref)), then
recurse the additive/multiplicative structure ([`recursive_find_arcs!`](@ref)). The per-tick entry
point that turns one side of a `:trans` line into the arc terms the step loop consumes.
"""
function extract_arcs(r_line, state::ReactionNetworkProblem)
    r_line = recursively_choose(r_line, state)

    return recursive_find_arcs!(
        escape_ref(r_line, state[:, :placeName]),
        1.0,
        Set{Symbol}(),
        Vector{FoldedArc}(undef, 0),
    )
end

"""
Parse a `@select(Kind, clause && clause …)` arc into a `(Kind::Symbol, TokenPredicate)` pair (ADR
...
"""
Recursively decompose one side of a reaction line into [`FoldedArc`](@ref) terms, accumulating the
...
a malformed line logs `@error`. Appends to and returns `arcs`.
"""
function recursive_find_arcs!(
        ex::SampleableValues,
        mult::SampleableValues,
        mods::Set{Symbol},
        arcs::Vector{FoldedArc},
    )
    if typeof(ex) != Expr || isexpr(ex, :.) || (ex.head == :escape)
        if (ex == 0 || in(ex, empty_set))
            return arcs
        else
            push!(arcs, FoldedArc(recursively_expand_dots(ex), mult, mods))
        end
    elseif ex.args[1] == :*
        recursive_find_arcs!(
            ex.args[end],
    ...
    elseif ex.args[1] == :+
        for i in 2:length(ex.args)
            recursive_find_arcs!(ex.args[i], mult, mods, arcs)
        end
    elseif ex.head == :macrocall && macroname(ex) == :select
        ...
        push!(arcs, FoldedArc(kind, mult, mods, pred))
    elseif isexpr(ex, :call) ||
            (ex.head == :macrocall && macroname(ex) ∈ [:structured, :move, :advance])
        push!(arcs, FoldedArc(ex, mult, mods))
    elseif ex.head == :macrocall
        mods = copy(mods)
        macroname(ex) in place_modalities && push!(mods, macroname(ex))
        foreach(
            i -> push!(mods, ex.args[i] isa Symbol ? ex.args[i] : ex.args[i].value),
            4:length(ex.args),
        )
        recursive_find_arcs!(ex.args[3], mult, mods, arcs)
    else
