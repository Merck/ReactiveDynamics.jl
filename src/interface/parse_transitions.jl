...
function expand_rate(rate)
    ...
    postwalk(rate) do ex
        if (isexpr(ex, :macrocall) &&
            (macroname(ex) ∈ prettynames[:transition][:cycle_time]))
            :(1 / $(ex.args[3]))
        else
            ex
        end
...
function get_transitions!(transitions, species, ex)
    ...
    rate, rline = exs[1:2]
    rline = parse_rline!(species, rline)
    rline = make_paths(rline, species)
...
function parse_rline!(species, rline)
    rline isa Expr && (rline.head == :-->) &&
        (rline = Expr(:call, :→, rline.args[1], rline.args[2]))

    if isexpr(rline, :macrocall)
        lines = copy(rline.args[3:end])
        rline.args = rline.args[1:2]
        for l in lines
            biarrow = nothing
        ...
        for i in 3:length(rline.args)
            rline.args[i] = if isexpr(rline.args[i], :tuple)
                Expr(:tuple, rline.args[i].args[1],
                     parse_rline!(species, rline.args[i].args[2]))
            else
                parse_rline!(species, rline.args[i])
            end
        end
    ...
    elseif rline isa Expr && rline.args[1] ∈ double_arrows
        biarrow = nothing
        prewalk(ex -> (ex ∈ double_arrows && (biarrow = ex); ex), rline)
        rline = parse_rline!.(Ref(species),
                              (replace_in_expr(rline, biarrow => :⟶),
                               replace_in_expr(rline, biarrow => :⟵)))
    end
...
function find_species!(species, ex)
    ...
    elseif ex.args[1] == :+
        for i in 2:length(ex.args)
            find_species!(species, ex.args[i])
        end
...
struct Species
    agent::Any # species identifier
    stoich::Any # stoich coeff can be a general expression
    modalities::Set{Symbol} # local modalities
end

"Split transition line into (LHS, RHS) tuple."
function split_rline(rline, network, this)
    if rline isa Expr && rline.args[1] ∈ fwd_arrows
        rline.args[2:3]
    elseif rline isa Expr && rline.args[1] ∈ bwd_arrows
        rline.args[[3, 2]]
        ...
        if isexpr(ex, :macrocall) && (macroname(ex) == :choose)
            choices = map(ex.args[3:end]) do r
                if isexpr(r, :tuple)
                    ...
                    recursively_choose(r, network, this)
                end
            end
...
function extract_species(rline, network, this)
    ...
    append_species!(rline, 1.0, Set{Symbol}(), Species[])
end
...
function append_species!(species::Vector{Species}, mult, mods::Set{Symbol}, ex)
    if typeof(ex) != Expr || (ex.head == :escape)
        if (ex == 0 || in(ex, empty_set))
            ...
            push!(species, Species(ex, mult, mods))
        end
    elseif ex.args[1] == :*
        append_species!(species, multiplex(mult, ex.args[2:(end - 1)]...), mods,
                        ex.args[end])
    elseif ex.args[1] == :+
        for i in 2:length(ex.args)
            append_species!(species, mult, mods, ex.args[i])
        end
    elseif isexpr(ex, :macrocall) && macroname(ex) == :action
        push!(species, Species(wrap_macro(ex), 1.0, Set{Symbol}()))
