...
```
"""
function transform(::Type{DiffEqBase.DiscreteProblem}, state::ReactiveDynamicsState; kwargs...)
    f = function (du, u, p, t)
        state = p[:__state__]
...
function DiffEqBase.DiscreteProblem(acs::ReactionNetwork, u0=Dict(), p=DiffEqBase.NullParameters(); kwargs...)
    ...
    state = ReactiveDynamicsState(acs, attrs, transitions, wrap_fun, keywords[:tspan][1]; keywords...); init_u!(state); save!(state)
