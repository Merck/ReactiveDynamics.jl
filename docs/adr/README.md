...
ADRs 0007–0010 are the Phase-0.5 modeling-language extension increment (2026-06-21), answering the maintainer's business-process modeling asks: a refined/compact/compositional metalanguage with substitutable granularity (0009), a contracted interface with declarative + dumpable initial state (0007), filtration-based selection of agentic-species tokens (0008), and an endogenous, state-contingent decision channel — in-model rules/callbacks/conditional transitions — that makes an expressive BD demo possible (0010). They map to CONTRACT §10, §11, §9.5, and §12 respectively. 0007–0009 are PROPOSED pending sign-off — every `file:line` is read-verified against `ref-agents`, but unlike 0001–0006 they have not yet had an adversarial running-engine verification pass. 0010 is maintainer-ratified (the three forks of 2026-06-21) and was driven by the [MVP_BD_DEMO.md](../MVP_BD_DEMO.md) exercise; its repair of the event channel (`solvers.jl:323`) is verified against source.
