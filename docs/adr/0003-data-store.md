...
> Maintainer rulings folded in (2026-06-18): (1) NO backward ACSet/on-disk compatibility required — the migration is free to break existing serialized models. (2) Phase 2 (promote the transition↔reactant relation to a typed `ReactantSpec` incidence table) is CONFIRMED, not optional. (3) The runtime store MUST support mutation during simulation — the append-only constraint and the live mutation API are specified in [ADR 0004](0004-runtime-mutation.md). (4) Serialization collapses to a single JSON format (dropping TOML/CSV/JLD2), specified in [ADR 0005](0005-serialization-json-ir.md). (5) Phase 3 (optional `to_acset` weakdep interop view) was initially OPTIONAL/low-priority and is now REJECTED / will-not-do (maintainer, 2026-07-15) — AlgebraicJulia interop is off the BD/rNPV roadmap and out of scope; see the Resolved section.
...
- Phase 2: promote the transition↔reactant relation to a first-class typed ReactantSpec incidence table with integer FKs (LENS A's structural upgrade), with an Expr escape-hatch for the legitimately dynamic @choose/@move/expression-valued-stoich reactants. This makes specs FK-checkable for agentic authoring, makes the relation round-trippable in TOML/JSON, and makes species-merge in equalize! structurally exact instead of fragile string substitution. (NB: `@structured` was originally listed in this escape-hatch set but is no longer an Expr escape hatch — its genesis form is now the typed, registry-resolved `@structured(:Kind, field = node, …)` reactant that round-trips through the JSON IR as a `structured{kind, fields}` row; the raw inline-constructor form was removed. See ADR 0005 §21 / 0006 Open questions.)
- ~~Phase 3 (optional): ship a tested to_acset/from_acset adapter behind a weakdep package extension (LENS D's interop hedge), preserving an optional AlgebraicPetri/Catlab view of the static spec at near-zero core cost.~~ **REJECTED / will-not-do (maintainer, 2026-07-15) — see the Resolved section.** There is no Phase 3; the migration is two phases, both landed.

If the maintainer chooses to stop after Phase 1, the terminal state is LENS B/D — a clean, conservative, dependency-free swap — which is itself fully defensible. (In the event, Phase 2 landed and Phase 3 was rejected, so the final state is LENS A: dependency-free typed IR with the promoted `ReactantSpec` relation, no interop view.)
...
- **Phase 3 (`to_acset`/`from_acset` weakdep interop adapter) → REJECTED / will-not-do (maintainer, 2026-07-15).** No ACSets/AlgebraicPetri/Catlab interop view will be built. AlgebraicJulia interop is off the BD/rNPV roadmap, is impossible for the running engine anyway (stateful in-flight `Transition` instances and expression-valued rates do not map to a plain Petri ACSet — it could only ever be a static-spec view), and maintaining a second representation plus its round-trip test is not justified for an unrequested capability. This supersedes the earlier 2026-06-20 "dropped/deferred, reversible" ruling: Phase 3 is now definitively out of scope, not a parked option. (Should formal AlgebraicPetri/AlgebraicDynamics interop ever become a goal it would be a NEW ADR against the then-current engine, not a revival of this phase — the dependency-free typed IR is the permanent core.) Consequently the phasing above is TWO phases (store swap, reactant promotion), both landed; there is no Phase 3.
