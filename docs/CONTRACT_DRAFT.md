...
### 9.5 Predicate selection of agentic tokens (see [ADR 0008](adr/0008-token-filtration.md))
...
**Phase-as-species vs phase-as-attribute — unified.** `get_species(a)==kind` is the degenerate clause `(species,==,kind)`, so `TokenPredicate` subsumes BOTH styles: phase-as-species (each phase a distinct KIND, `@move`-shuttled — ADR 0006 north-star) is filtration on the `species` field; phase-as-attribute (one `:Project` KIND with a `phase` field, advanced by `SetAttr`, selected by `@select`) is filtration on `phase`. They become one bind code path, not two mechanisms. RECOMMENDED BD-template default: phase-as-attribute (no kind explosion; continuous predicates like `npv>θ` work; avoids `@move`'s redundant re-`entangle!` `solvers.jl:376`); phase-as-species stays available. Note the bind/read split: `TokenPredicate` consumes/holds (CONTRACT §1 modality applies, `perstep` still illegal for structured species §1.4); `TokenAgg` reads non-consumingly — both share the predicate vocabulary, never alias.
...
The Phase-0 modeling contract is COMPLETE and SIGNED OFF (§1–§9): §1 modality truth table, §2 time model (+§2.8 genesis modes), §3 operational semantics, §4 determinism & seeding, §5 attribute contract, §6 object model, §7 composition semantics, §8 serialization schema, §9 structured tokens & queries (+§9.5 predicate selection). A Phase-0.5 modeling-language extension increment adds §10 interface & initial-state contract and §11 refinement & open-port composition, with §9.5 extending §9 — the business-process modeling refinements (compact/compositional authoring, granularity/substitution, declarative initial state + state dump, and agentic-species filtration). It is backed by [ADR 0001](adr/0001-discrete-event-engine.md) (engine), [ADR 0002](adr/0002-priority-weighted-allocation.md) (allocation), [ADR 0003](adr/0003-data-store.md) (data store), [ADR 0004](adr/0004-runtime-mutation.md) (runtime mutation), [ADR 0005](adr/0005-serialization-json-ir.md) (JSON serialization), [ADR 0006](adr/0006-structured-tokens.md) (structured tokens + custom-function registry), [ADR 0007](adr/0007-interface-and-initial-state.md) (interface & initial state + dump/restore), [ADR 0008](adr/0008-token-filtration.md) (token filtration / predicate selection), and [ADR 0009](adr/0009-refinement-and-composition.md) (refinement & open-port composition). Every `file:line` citation in §1–§9 was adversarially verified against the current `ref-agents` source (several verifiers ran the engine on Julia 1.12.5); the §10/§11/§9.5 citations are read-verified against the same tree and PROPOSED pending maintainer sign-off. The Phase-0 semantic test suite that encodes the §1–§9 invariants lives under `test/semantic/`; the §10/§11/§9.5 acceptance tests are to be added with their ADRs. With maintainer sign-off this closes the Phase-0 gate; implementation (Phase 1) may then begin.
