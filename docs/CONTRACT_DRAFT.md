...
The ADR-0005 action set `{SetSpecies, SetParams, Log, Seq}` is extended to the full endogenous action *type family* `{SetSpecies, SetParams, SetField, AddToken, Activate, Deactivate, Log, Seq}` (`ACTION_VERBS = (:set_species, :set_params, :set_field, :add_token, :activate, :deactivate, :log, :seq)`). `SetSpecies` carries a `mode ∈ {set, inc}` (the `inc` form is the cash/capital lever). `SetField{field, value}` (§9.5, ADR 0008 §D) writes a field of the firing instance's bound token(s) — the per-token counterpart of `SetSpecies` and the mutate-existing counterpart of `AddToken`; it is how a program advances phase (`@advance(phase, :Phase3) ⇒ SetField{:phase, …}`), and `@move(:from,:to)` is its degenerate `SetField{:species, …}` sugar. `AddToken{kind, fields}` lowers to `add_structured_token!(state, registry[kind](…))` — `entangle!`, append-only/index-safe (§9.1) — and is the in-model acquisition (CREATE a token, vs `SetField` which MUTATES one). `Activate`/`Deactivate{transition}` flip `transActivated` (ADR 0004 soft-deactivate; in-flight instances still finish, `solvers.jl:406`). **Context restriction:** `SetField` writes the firing transition instance's bound token, so it is legal ONLY in a transition **post-action**; a standalone Rule (fired at step 10 with no transition instance) has no bound token, so a **Rule action** uses the system-/pool-level subset `{SetSpecies, SetParams, AddToken, Activate, Deactivate, Log, Seq}` and `SetField` in a Rule is a `validate()` diagnostic. All verbs lower eval-free into the closed allow-list extended by the ADR-0006 registry key set; an unregistered `AddToken.kind`, an unknown `Activate`/`Deactivate` target, an unknown `SetField.field`, an out-of-set verb, a `SetField` outside a transition post-action, or a `Ref` to an undeclared name is a `validate()` diagnostic, never an eval (§8.4 S4 preserved). A program-advancing action that moves a structured token between phases is `@advance(phase, …)` (`SetField` on the `phase` field, §9.5 phase-as-attribute), not a `SetSpecies` (which writes a plain `state.u` count, not a token field).
...
The Phase-0 modeling contract is COMPLETE and SIGNED OFF (§1–§9): §1 modality truth table, §2 time model (+§2.8 genesis modes), §3 operational semantics, §4 determinism & seeding, §5 attribute contract, §6 object model, §7 composition semantics, §8 serialization schema, §9 structured tokens & queries (+§9.5 predicate selection). A Phase-0.5 modeling-language extension increment adds §10 interface & initial-state contract, §11 refinement & open-port composition, and §12 rules/triggers & conditional transitions, with §9.5 extending §9 — the business-process modeling refinements (compact/compositional authoring, granularity/substitution, declarative initial state + state dump, agentic-species filtration, and the endogenous decision channel that makes an expressive BD demo possible). It is backed by [ADR 0001](adr/0001-discrete-event-engine.md) (engine), [ADR 0002](adr/0002-priority-weighted-allocation.md) (allocation), [ADR 0003](adr/0003-data-store.md) (data store), [ADR 0004](adr/0004-runtime-mutation.md) (runtime mutation), [ADR 0005](adr/0005-serialization-json-ir.md) (JSON serialization), [ADR 0006](adr/0006-structured-tokens.md) (structured tokens + custom-function registry), [ADR 0007](adr/0007-interface-and-initial-state.md) (interface & initial state + dump/restore), [ADR 0008](adr/0008-token-filtration.md) (token filtration / predicate selection), [ADR 0009](adr/0009-refinement-and-composition.md) (refinement & open-port composition), and [ADR 0010](adr/0010-rules-and-conditional-transitions.md) (rules/triggers & conditional transitions). The [MVP_BD_DEMO.md](MVP_BD_DEMO.md) acquisition-impact demo is the exercise that drove §12 and catalogues the contract weak-points it surfaced; the Phase-0.5 increment resolved findings B (§12), C (§9.5), F (§11), H (§10.3), leaving open A (counterfactual RNG, §4.6 future work), D / D-bis (per-program ledger + TVE-freezing semantics), G (per-tick token KPIs need `getobservable`), and a new finding I (a Rule action that writes a token field over a `@select`-ed set — the `SetField`/`@advance` verb of ADR 0008 should extend the §12.3 action set). Every `file:line` citation in §1–§9 was adversarially verified against the current `ref-agents` source (several verifiers ran the engine on Julia 1.12.5); the §10/§11/§12/§9.5 citations are read-verified against the same tree and PROPOSED pending maintainer sign-off. The Phase-0 semantic test suite that encodes the §1–§9 invariants lives under `test/semantic/`; the §10/§11/§12/§9.5 acceptance tests are to be added with their ADRs. With maintainer sign-off this closes the Phase-0 gate; implementation (Phase 1) may then begin.
