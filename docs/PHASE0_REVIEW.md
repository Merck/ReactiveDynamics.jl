...
>
> **Phase-0.6 analysis & visualization increment — PROPOSED 2026-06-29 (§9 of this summary).** Two additive ADRs (0013–0014) and contract sections (§14, §15) answer the maintainer's result-inspection asks: inspect an ensemble of runs, gather ledger statistics, extract "typical" agentic-token trajectories, export results, plot via model-agnostic recipes, and draw a result-decorated network "exec map" for (in)efficiency analysis. The layer is read-only over a finished run and reuses existing infrastructure (the per-program-ledger seam; AA's `getobservable`/`run_graphviz`/`wiring_diagram`) rather than adding dependencies; it promotes the BD demo's hand-rolled `analysis.jl`/`figures.jl` into engine APIs. Read-verified against `ref-agents` (and the AlgebraicAgents source); awaiting sign-off.
...
| [0013](adr/0013-analysis-and-observability.md) | **Analysis & observability**: per-token trajectory log (orchestrator-driven `log_token_fields` hook + columnar store, generalizing the per-program ledger row); ensemble runner (`EnsembleProblem` as an AA node, `summarize`/`treatment_effect`); results export bundle (CSV/JSON core, Arrow weakdep). | Proposed 2026-06-29 |
...
## 9. Phase-0.6 — the analysis & visualization increment (proposed 2026-06-29)
