...
#
# **This is a topic-driven deep-dive, not a cumulative tutorial.** Each section is self-contained and seed-pinned. For the end-to-end structured-token modeling workflow these constructs come from, see the [advanced tutorial](../tutorials/advanced.md); for the exact document shape, the [JSON model schema](../reference/json_schema.md) reference; for the full API, the [serialization reference](../reference/serialization.md); and for the *why* behind the trust boundary, the [Models as data](../explanation/models_as_data.md) explanation page.
