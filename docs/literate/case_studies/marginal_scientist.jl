...
# - The two trials carry different **`priority`** weights. When more trials are ready than the bench can staff in a tick, the [priority-weighted allocator](../explanation/allocator.md) rations the scientists in proportion to priority. We give oncology the higher priority (protecting the higher-strategic-value franchise), so under contention oncology is staffed first and immunology waits — a queue we will *see* in §3.
#
