...
#
# - `network_graph(prob)` — the pure Petri-net structure: places, transitions, and the arcs between them, with multiplicity and modality. It runs on a copy and never perturbs the run.
