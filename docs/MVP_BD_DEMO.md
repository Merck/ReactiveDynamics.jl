...
> **Status — Phase-0.5 design artifact, 2026-06-21.** A demo specification written against the contract ([CONTRACT_DRAFT.md](CONTRACT_DRAFT.md) §1–§12) and ADRs 0001–0010. Its second purpose is adversarial: to surface limitations of the draft contract *before* Phase-1 implementation. Findings are catalogued in §8; the endogenous-decision-channel finding (B) has been promoted to [ADR 0010](adr/0010-rules-and-conditional-transitions.md) + CONTRACT §12, and the concurrent Phase-0.5 increment (ADRs 0007/0008/0009 → CONTRACT §10/§9.5/§11) resolves or softens findings C, F, G and the declarative-initial-portfolio gap (cross-referenced inline). Domain is pharma throughout (the engine's demonstrators are pharma; rNPV is the canonical BD metric), but the structure is domain-general.
...
**H. Structured-token initial state was not declarative. — RESOLVED (§10.3 / ADR 0007).** The starting portfolio was built by imperative host code (`add_structured_token!` in a loop), so it lived outside the model document and broke §8.2 S2 ("(model.json, seed) determines the run") for structured models. §10.3 adds a declarative serializable `population[]` array (the structured analogue of `specInitVal`), instantiated at construction before t=0 with seeded attribute draws and per-species creation indices. The MVP's initial pipeline is now reproducible input.

**I. A Rule action cannot yet write a token field over a `@select`-ed set. — OPEN, NARROW (the one new gap; partially covered by ADR 0008's `SetField`/`@advance`).** The maintainer's §9.5 ruling makes `phase`, `npv`, `pos_remaining`, … TOKEN ATTRIBUTES, and ADR 0008 adds `SetField`/`@advance` to write a token field — but as a **transition-RHS** idiom (the advance on success), not as a **Rule action verb**. The ADR-0010 §12.3 action set `{SetSpecies, SetParams, AddToken, Activate, Deactivate, Log, Seq}` writes only plain species and params, so an endogenous *rule* still cannot perturb the token population — e.g. "on a competitor readout, write down all Phase-2 oncology programs' `pos_remaining` by 10%." *Recommendation:* lift ADR-0008's `SetField` into the §12.3 closed action set as `SetField{predicate::TokenPredicate, field, value::ExprNode}` (a population-level write over a §9.5 selection), so the rule channel and the token-field-write channel compose. Small, additive, eval-free; flagged as a pre-Phase-1 ADR-0010/ADR-0008 amendment (recorded in both ADRs' open questions).
...
2. **Decision channel:** endogenous rules/callbacks + conditional transitions are **core** — promoted to ADR 0010 + CONTRACT §12 (finding B). Ratified forks: guard field **and** rule-driven Activate/Deactivate; full action set `{SetSpecies, SetParams, AddToken, Activate, Deactivate, Log, Seq}`; `fire_mode {every_tick, once}`.
3. **Phase representation:** phase-as-attribute is canonical (§9.5/ADR 0008 §D); `@select` selects, `@advance`/`SetField` advances.
4. **Domain:** pharma throughout.

## 10. Next steps

- (this round) ADR 0010 + CONTRACT §12 + §4.6 written; ADR index updated; findings re-statused against the Phase-0.5 increment (B/C/F/H resolved, G partial, A/D/D-bis open, new finding **I** raised).
- (before Phase-1) resolve finding **I** — add a token-field-write action verb (`SetField`/`SetTokens` over a `TokenPredicate`) to the §12.3 / §9.5 surface, so endogenous decisions can perturb the token population.
- (Phase-1, gated) implement the ADR-0010 surface + the §7 prerequisite bug fixes; then scaffold `host/ProjectToken.jl` + `models/pipeline.rdj.json` + `analysis/rnpv.jl` so each §8 finding becomes an executable test against the contract.
