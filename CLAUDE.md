...
- Old names survive ONE release as `@deprecate`/`@deprecate_binding` shims (`src/ReactiveDynamics.jl:439-457`; the deprecated `@ReactionNetworkSchema` macro alias at `src/interface/create.jl:83`). GeneratedExpressions was dropped.
...
Tests live in `test/semantic/*.jl` (entry `test/runtests.jl` → `test/semantic/runtests.jl`) under a two-tier scheme: **T1-characterization** (runs against the engine) and **T2-acceptance** (target behavior). The suite is green with no `@test_broken` pins (the last, the §1.4 modality validators, landed in `53d1fac`) and no `@test_skip` PLACEHOLDERS — but three `@test_skip`s remain as ENVIRONMENT guards, so the tally depends on the environment: **835 pass / 0 broken / 835 total** locally when a Graphviz backend is installed, **833 pass / 1 broken (environment-skipped) / 834 total** on CI. The one skip CI takes is `test/semantic/visualization.jl:132`, guarding on a Graphviz backend — a SYSTEM BINARY, not a Julia package, so it is absent on the runners. The other two guards (`test/semantic/visualization.jl:203` Plots, `test/semantic/analysis_observability.jl:333` Arrow) do NOT trip on CI, because Plots and Arrow are declared in `test/Project.toml`. `test/Project.toml` declares the test-only deps (Plots, Arrow, DataFrames, Distributions, …) that the main project keeps as weakdeps. AlgebraicAgents is the registered 0.4 release (no `[sources]` pin). Julia ≥ 1.12.

Formatting is [Runic](https://github.com/fredrikekre/Runic.jl) (opinionated, non-configurable — no config file). Run `julia -m Runic --inplace src test ext docs/make.jl demo` before committing, or `julia -m Runic --check .` to verify. CI **is** configured (`.github/workflows/`: `CI.yml`, `Runic.yml`, `CompatHelper.yml`, `TagBot.yml`) and gates every PR: the `test` matrix (Julia 1 and 1.12 × ubuntu/macOS/windows), `Downgrade compat`, `Documentation` (builds the site and deploys a PR preview), and `Runic formatting`. `Julia nightly - ubuntu-latest` is `continue-on-error: true` (`CI.yml:53`) — informational, allowed to break. Nothing runs `demo/`, so demo rot is invisible to CI; still run the suite and formatter locally before pushing. The bulk reformat commit is listed in `.git-blame-ignore-revs`; `git config blame.ignoreRevsFile .git-blame-ignore-revs` makes `git blame` skip it.
...
- **Adding a new `src/` file** requires clearing the compiled cache, or Revise won't pick it up. (The `dev/` Revise test server that used to soften this does NOT exist on this branch — there is no `dev/` directory.)
